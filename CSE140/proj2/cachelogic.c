  ...
  /* Start adding code here */
    ...
    for(int i = 0; i < assoc; i++) {
        ...
        if(cache[indexValue].block[i].valid == VALID && cache[indexValue].block[i].tag == tagValue) {
            ...
            //If it's a write operation and the policy is write through we also want to change the memory
            if(we == WRITE && memory_sync_policy == WRITE_THROUGH){
                accessDRAM(addr, (cache[indexValue].block[i].data), tu, WRITE);
            }
            //If it's a write operation and the policy is write back we need to update the dirty bit
            else if(we == WRITE && memory_sync_policy == WRITE_BACK) {
                cache[indexValue].block[i].dirty == DIRTY;
            }
            break;
    ...
    if(hit == false) {
        ...
        if(cache[indexValue].block[toBeReplaced].dirty == DIRTY && memory_sync_policy == WRITE_BACK) {
            ...
            accessDRAM(memAddr, (cache[indexValue].block[toBeReplaced].data), tu, WRITE);
