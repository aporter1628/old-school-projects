...
int matches_leading(char *partial_line, char *pattern) {
    ...
    while(*(pattern) != '\0' && *(partial_line) != '\0') {
        ...
        if(*(pattern) == '\\') {
            ...
            if(*(pattern + 2) == '+') {
                //If there's a question mark after the plus
                if(*(pattern + 3) == '?') {
                    if(*(pattern + 1) != *(partial_line)) {
                        ...
                        if(*(pattern + 4) == *(pattern + 1)) {
                            ...
                            count2 = 0;
                            while(*(pattern + 4 + count2) == *(pattern + 1)) {
                                count2++;
                            }
...
int rgrep_matches(char *line, char *pattern) {
    ...
    for(int i = 0; *(line+i) != '\0'; i++) {
        ...
        if(*pattern == '\\') { 
            ...
            //after it
            if(*(pattern + 2) == '?') {
                //If the pattern ends after the question mark there is no other character, so anything will work
                if(*(pattern + 3) == '\0') 
                    return 1;
                else{
                    pattern += 3;
